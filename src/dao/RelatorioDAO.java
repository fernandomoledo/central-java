...
public class RelatorioDAO {
	...
	public Relatorio getDados(String mes, String ano) throws SQLException, ClassNotFoundException, NamingException{
		...
		sql = "select count(distinct(c.id)) as qtde from chamados c, andamentos a, chamado_palavra cp, palavra_chave p where "+
				...
				" c.id = cp.chamado and cp.palavra = p.id and p.lotacao in (SELECT l.id FROM   sada.lotacao l where  (l.mae in (631,1301,1316) or l.id in (631)) "+
		...
		sql = "select count(*) from( "+                     
			...
			" select  "+
			" distinct(c.id) "+
			" from chamados c, andamentos a, chamado_palavra cp, palavra_chave p where "+
			" c.id = a.chamado and a.classificacao='ABE' and to_char(a.dt_andamento,'MM/YYYY') = ? and "+
			" c.id = cp.chamado and cp.palavra = p.id and "+
			" p.lotacao in (SELECT l.id "+
			                     "   FROM   sada.lotacao l "+
			                     "   where  (l.mae in (631,1301,1316) "+
			                     "   or     l.id in (631)) "+
